...
export class TimestampGenerator {
  ...
  generateEventTimes(cardCreationTime, count = 10) {
    ...
    for (let i = 0; i < count; i++) {
      // 이전 일정으로부터 3~21일 후
      let startDate = this.generateRealisticTimestamp(lastEventTime, 3, 21);
      
      // 고유성 보장: 이전 일정과 최소 1주일 이상 차이
      if (i > 0 && events.length > 0) {
        const prevStartTime = new Date(events[events.length - 1].startDate);
        const minDiff = 7 * 24 * 60 * 60 * 1000; // 1주일 (7일)
        if (startDate.getTime() - prevStartTime.getTime() < minDiff) {
          startDate = new Date(prevStartTime.getTime() + minDiff + Math.random() * 7 * 24 * 60 * 60 * 1000);
          // 비즈니스 시간으로 재조정
  ...
  ensureUnique(date) {
    ...
    while (this.usedTimestamps.has(newDate.getTime()) && attempts < 100) {
      // 최소 1분씩 뒤로 밀기 (밀리초 단위로도 차이 보장)
      const offset = (1 + Math.random()) * 60 * 1000; // 1-2분
